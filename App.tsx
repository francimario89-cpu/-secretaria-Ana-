...
const Dashboard = () => (
  <div className="p-6 lg:p-12 max-w-5xl mx-auto">
    <header className="mb-10 animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      <p className="text-slate-500 mt-1 font-medium">Sua secretária pessoal já está de prontidão para hoje.</p>
    </header>

    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
      <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
        <p className="text-slate-400 text-xs font-bold uppercase mb-1 tracking-wider">Saldo em Conta</p>
      ...
      <div className="bg-emerald-500 p-8 rounded-[2rem] text-white shadow-xl relative overflow-hidden group">
        <p className="text-emerald-100 text-[10px] font-bold uppercase mb-1 tracking-widest">Status da Secretária</p>
        <p className="text-xl font-bold leading-tight">"Aguardando seus comandos no WhatsApp!"</p>
    ...
    <Link to="/chat" className="block bg-emerald-50 border-2 border-dashed border-emerald-200 p-12 rounded-[2.5rem] text-center hover:bg-emerald-100/50 hover:border-emerald-400 transition-all group">
      <div className="flex flex-col items-center gap-2">
        <i className="fab fa-whatsapp text-4xl text-emerald-500 group-hover:scale-110 transition-transform"></i>
        <p className="text-emerald-800 font-black text-lg">CLIQUE PARA FALAR COM A ANA</p>
        <p className="text-emerald-600/60 text-xs font-bold uppercase">Simulação de chat em tempo real</p>
      </div>
    </Link>
  </div>
);
...
const Chat = () => {
  ...
  useEffect(() => {
    localStorage.setItem('ana_chat_v3', JSON.stringify(messages));
